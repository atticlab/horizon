...
func (ingest *Ingestion) UpdateAccountIncome(address string, assetCode string, income int64, ledgerClosedAt time.Time, now time.Time) error {
    return ingest.updateAccountStats(address, assetCode, income, ledgerClosedAt, now, true)
}

// UpdateAccountOutcome updates outcome stats for specified account and asset
func (ingest *Ingestion) UpdateAccountOutcome(address string, assetCode string, outcome int64, ledgerClosedAt time.Time, now time.Time) error {
    return ingest.updateAccountStats(address, assetCode, outcome, ledgerClosedAt, now, false)
}
...
func (ingest *Ingestion) updateAccountStats(
	...
	stats, err := ingest.getStats(address, assetCode)
...
func (ingest *Ingestion) getStats(address string, assetCode string) (history.AccountStatistics, error) {
    var stats history.AccountStatistics
    sql := history.SelectAccountStatisticsTemplate.Limit(1).Where("a.address = ? AND a.asset_code = ?", address, assetCode)
...
func (ingest *Ingestion) updateStats(stats history.AccountStatistics) error {
    ...
    sql = sql.Where("address = ? AND asset_code = ?", stats.Account, stats.AssetCode)
