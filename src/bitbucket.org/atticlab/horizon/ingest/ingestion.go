...
func (ingest *Ingestion) incrementAccountColumn(address string, column string, value string) error {
	sql := fmt.Sprintf("UPDATE history_accounts SET %s = %s + $1 WHERE address = $2", column, column)

	_, err := ingest.DB.ExecRaw(sql, value, address)
	if err != nil {
		return err
	}
...
// UpdateAccountIncome Updates daily, weekly, monthly and annual income stats
func (ingest *Ingestion) UpdateAccountIncome(address string, income string, timestamp time.Time) error {
	utcNow := time.Now().UTC()
	utcTime := timestamp.UTC();
	
	var err error
	if utcNow.Year() == utcTime.Year() {
		println("Income: this year")
		err = ingest.incrementAccountColumn(address, "annual_income", income)
		
		if utcNow.Month() == utcTime.Month() {
			println("Income: this month")
			err = ingest.incrementAccountColumn(address, "monthly_income", income)
			
			year1, week1 := utcNow.ISOWeek()
			year2, week2 := utcTime.ISOWeek()
			
			if  week1 == week2 && year1 == year2   {
				println("Income: this week")
				err = ingest.incrementAccountColumn(address, "weekly_income", income)
				
				if utcNow.Day() == utcTime.Day() {
					println("Income: this day")
					err = ingest.incrementAccountColumn(address, "daily_income", income)
				}
			}
		}
	}
...
// UpdateAccountOutcome Updates daily, weekly, monthly and annual outcome stats
func (ingest *Ingestion) UpdateAccountOutcome(address string, outcome string, timestamp time.Time) error {
	now := time.Now().Local()
	localTimestamp := timestamp.Local();
	
	var err error
	if now.Year() == localTimestamp.Year() {
		println("Outcome: this year")
		err = ingest.incrementAccountColumn(address, "annual_outcome", outcome)
		
		if now.Month() == localTimestamp.Month() {
			println("Outcome: this month")
			err = ingest.incrementAccountColumn(address, "monthly_outcome", outcome)
			
			year1, week1 := now.ISOWeek()
			year2, week2 := localTimestamp.ISOWeek()
			
			if  week1 == week2 && year1 == year2   {
				println("Outcome: this week")
				err = ingest.incrementAccountColumn(address, "weekly_outcome", outcome)
				
				if now.Day() == localTimestamp.Day() {
					println("Outcome: this day")
					err = ingest.incrementAccountColumn(address, "daily_outcome", outcome)
				}
			}
		}
	}
