...
func TestProcessedOp(t *testing.T) {
	...
	Convey("Does not exist", t, func() {
		...
		So(err, ShouldBeNil)
		processedOp, err := processedOpProvider.Get(account.Address(), 1)
		So(err, ShouldBeNil)
	...
	Convey("Delete", t, func() {
		...
		processedOp := NewProcessedOp(txHash, rand.Int(), rand.Int63(), time.Unix(time.Now().Unix(), 0))
		err = processedOpProvider.Insert(processedOp, time.Duration(5)*time.Second)
		So(err, ShouldBeNil)
		stored, err := processedOpProvider.Get(processedOp.TxHash, processedOp.Index)
		assert.Equal(t, processedOp, stored)
		err = processedOpProvider.Delete(processedOp.TxHash, processedOp.Index)
		So(err, ShouldBeNil)
		stored, err = processedOpProvider.Get(processedOp.TxHash, processedOp.Index)
		So(err, ShouldBeNil)
	...
	Convey("Storing", t, func() {
		...
		txHash := account.Address()
		opIndex := rand.Int()
		processedOp := NewProcessedOp(txHash, opIndex, rand.Int63(), time.Unix(time.Now().Unix(), 0))
		expireTime := time.Duration(2) * time.Second
		err = processedOpProvider.Insert(processedOp, expireTime)
		So(err, ShouldBeNil)
		storedProcessedOp, err := processedOpProvider.Get(txHash, opIndex)
		So(err, ShouldBeNil)
